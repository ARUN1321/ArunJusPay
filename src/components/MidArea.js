...
export default function MidArea({ spirit, setSpirit, spiritActs }) {
  ...
  }, [updatedPath, spiritActs.name, setSpirit]);
  ...
  const updateInputBlockValue = (type, index, updatedFields) => {
    setUpdatedPath((prev) => {
      ...
      if (type?.name === "repeateChild") {
        target.action.children[type.childIndex].action = {
          ...target.action.children[type.childIndex].action,
          ...updatedFields,
        };
      } else {
  ...
  return (
    <div
      className="flex-1 relative h-full overflow-auto p-2 flex items-center justify-center text-center"
    ...
    >
      <div className="border-2 border-dotted rounded-md border-black w-full h-full bg-blue-50">
        ...
        <div
          ref={drop}
          className={`w-full flex flex-col items-start pt-10 pl-10 ${
            isOver ? "bg-green-100" : ""
          }`}
            ...
            <div key={index} className="relative mt-1 mb-1">
              <div
                className={`inline-flex items-center justify-center ${
                  ele.action.active === "Repeat" ? "h-full" : "h-8"
                } ${ele.color} text-white rounded-md px-3 py-2 text-sm font-bold m-1`}
              >
                <ActionBlockRenderer
                  ele={ele}
                  index={index}
